...
export const extractSingleUKPostcode = (address) => {
    // Enhanced regex to match UK postcodes in various formats
    const postcodeRegex = /([A-Za-z][A-HJ-Ya-hj-y]?[0-9][0-9A-Za-z]?[ ]?[0-9][A-Za-z]{2}|GIR[ ]?0AA)/i;

    // Find the first valid match in the string
    const match = address.match(postcodeRegex);

    if (!match) return null;

    // Format the postcode (uppercase with space)
    const rawPostcode = match[0];
    const cleaned = rawPostcode.replace(/\s+/g, '').toUpperCase();
