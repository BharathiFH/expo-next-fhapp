...
import { getBaseURL } from 'appmodules/ConfiguratorModule/Utils/ConfiguratorHelper';
...
import { SERVER_ERROR_CONSTANTS } from '../../AppModules/ErrorModule/Utils/ErrorModuleConstants';
...
export const requestHandler = (request) => {
    ...
    //Removed for menu uplift feature
    ...
    // !isWeb() && (request.headers.cancelToken = source.token); //excluding temporarily for web to avoid CORS issue.
    return request;
};
...
export const errorHandler = (error) => {
    ...
    if (isValidElement(error?.response?.data)) {
        ...
        if (
            ...
            errorObject.code = isValidElement(data.error) ? data?.error?.code : error?.response?.status ?? '';
            errorObject.path = isValidElement(error?.request?.responseURL) ? error?.request?.responseURL : error?.config?.url ?? '';
            errorObject.headers = {};
            if (isValidElement(error?.config?.headers)) {
                errorObject.headers = { ...error?.config?.headers };
                delete errorObject.headers['api-token'];
                delete errorObject.headers.deviceInfo;
                delete errorObject.headers.Authorization;
            }
            errorObject.payload = isValidElement(error?.config?.data) ? error?.config?.data : '';
            errorObject.responseData = {};
            if (isValidElement(error?.response?.data)) {
                const responseData =
                    typeof error?.response?.data === 'string' ? SERVER_ERROR_CONSTANTS.HTML_CONTENT : error?.response?.data;
                errorObject.responseData = {
                    data: responseData,
            ...
            errorObject.status = isValidElement(error?.response?.status) ? error?.response?.status : '';
