import axios from 'axios';
import { errorHandler, requestHandler, successHandler } from './NetworkInterceptors';
import { AppConfig, BASE_URL } from '../../CustomerApp/Utils/AppConfig';
import { Platform } from 'react-native';
import Config from 'react-native-config';
import { getAppType } from '../../AppModules/ConfiguratorModule/Utils/ConfiguratorHelper';

export const BASE_API_CONFIG = {
    baseURL: BASE_URL,
    applicationName: getAppType(),
    requestTimeOut: 20000,
    maximumRequestTimeOut: 40000
};
/**
 *  Product id for Customer App - 1 Foodhub - 3
 * @type {{consent_by: string, product_id: number, platform_id: number, platform: (string), consent_type: string, policy_type_id: {terms_and_conditions: number, privacy_policy: number, sms: number, terms_of_use: number, all_promo: number, email: number, allergy_information: number,support:number,pushNotification:number}}}
 */
export const BASE_PRODUCT_CONFIG = {
    platform_id: Platform.OS === 'web' ? 1 : Platform.OS === 'ios' ? 2 : 3,
    product_id: Config.PRODUCT_ID,
    gdpr_Platform_id: Config.GDPR_PLATFORM_ID,
    consent_by: 'CUSTOMER',
    consent_type: 'CUSTOM',
    platform: Platform.OS,
    policy_type_id: {
        terms_and_conditions: 1,
        terms_of_use: 5,
        privacy_policy: 4,
        allergy_information: 2,
        sms: 7,
        pushNotification: 9,
        email: 8,
        all_promo: 3,
        about_us: 6,
        support: 12,
        guest_privacy_policy: 16,
        cookies_policy: 10
    },
    OPEN_API_TOKEN: '99b8ad5d2f9e80889efcd73bc31f7e7b'
};

const axiosInstance = axios.create({
    baseURL: BASE_API_CONFIG.baseURL,
    timeout: BASE_API_CONFIG.requestTimeOut,
    headers: { 'api-token': AppConfig.API_TOKEN }
});

// Add interceptors
axiosInstance.interceptors.request.use((request) => requestHandler(request));
axiosInstance.interceptors.response.use(
    (response) => successHandler(response),
    (error) => errorHandler(error)
);

export default axiosInstance;
